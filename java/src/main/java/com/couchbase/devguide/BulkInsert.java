        ...
        Observable<JsonDocument> asyncProcessing = Observable
                ...
                .flatMap(new Func1<JsonDocument, Observable<JsonDocument>>() {
                    public Observable<JsonDocument> call(JsonDocument doc) {
                        // System.out.println("flatmap " + System.currentTimeMillis());
                        // if (doc.id().endsWith("3"))
                        //     return bucket.async().upsert(doc).delay(3, TimeUnit.SECONDS); //artificial delay for item 3
                        // if (doc.id().endsWith("4"))
                        //     return bucket.async().upsert(doc).delay(5, TimeUnit.MILLISECONDS); //artificial delay for item 3
                        return bucket.async().upsert(doc);
                    }
                })
                .timeout(8,TimeUnit.MILLISECONDS)
                .onErrorReturn(new Func1<Throwable, JsonDocument>() {
                    @Override
                    public JsonDocument call(Throwable throwable) {
                        if (throwable instanceof TimeoutException) {
                            LOGGER.info("timeout");
                            return JsonDocument.create("user::anonymous",
                                JsonObject.empty().put("firstname", "john-doe"));
                        }
                        return JsonDocument.create("user::anonymous",
                            JsonObject.empty().put("firstname", "john-doe"));
                    }
